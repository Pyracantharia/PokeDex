    ...
    if ($errorId) {
        ...
        if (isset($_POST['pseudo'], $_POST['message'], $_POST['nom'], $_POST['types'], $_POST['num'], $_POST['taille'], $_POST['poids'], $_POST['talent'], $_POST['couleur'])) {
            ...
            if ($errorForm) {
                ...
                $query = $db->getPdo()->prepare("UPDATE livredor SET username = :username, contenu = :contenu, date = :date, nom = :nom, types = :types, num = :num, taille = :taille, poids = :poids, talent = :talent, couleur = :couleur WHERE id = :id");
                $query->execute([
                    'id' => $id,
                    'username' => $username,
                    'contenu' => $contenu,
